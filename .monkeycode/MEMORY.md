...
- Instructions:
  ...
  - E2E mock：llm-providers 需含 scenes/scene_labels/priority，/api/status 需含 scene_models（五通道 fast/clinic/emergency/wellness/maternal）；admin-e2e 断言含通道徽章、启用/停用、弹窗场景多选。全套 jsdom E2E 位于 /tmp/opencode：nav/channels/main/member/admin/maternity/acct-debug/pay/purchase-pay/pay-crypto/navcheck，运行 `cd /tmp/opencode && node <name>.mjs`，需后端 3001 与 client dev 5173 在跑；channels/maternity/acct 已带断言并按当前模型（GLM-4.5-Flash / DeepSeek V4-Pro / Qwen3.8-Max / Spark-X2.5）校验，退出码非 0 即失败。换路由会重建 DOM，测试轮询时每轮重新查询 #member-content，不要持有旧引用。
