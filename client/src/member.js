...
async function renderAccount(el) {
  ...
  el.innerHTML = `
    <div class="member-head"><h2>账号设置</h2><p>维护登录账号资料与密码。</p></div>
    <div class="block">
      <h3>基本资料</h3>
      <div class="field"><label>登录手机号</label><input id="ac-phone" type="tel" maxlength="11" value="${esc(m.phone || "")}" placeholder="新手机号（留空则不修改）" /></div>
      <div class="field"><label>称呼</label><input id="ac-name" type="text" maxlength="20" value="${esc(m.name || "")}" /></div>
      <div class="field"><label>当前密码（确认修改）</label><input id="ac-cur1" type="password" maxlength="64" placeholder="请输入当前密码" /></div>
      <div class="error" id="ac-error"></div>
      <button class="btn btn-primary" id="ac-save">保存资料</button>
    </div>
    <div class="block">
      <h3>修改密码</h3>
